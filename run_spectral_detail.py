...
OUTPUT_DIR          = paths.output_dir("run_spectral_detail.py")
SCORE_TXT_DIR       = paths.SCORE_TXT_DIR

MAX_TRAIN_BONA = None
MAX_EVAL       = None

REMAKE_CACHE_SPECTRAL_TRAIN = True
REMAKE_CACHE_SPECTRAL_EVAL  = False
...
def extract_spectral_detail(wav: torch.Tensor):
    """
    SpectralDetail-Hvr  = mean_t( log|X_fine| - log|X_blurry_upsampled| )
    Returns a (FINE_BINS,) utterance-level vector.
    """
    log_fine = _log_mag(wav, FINE_FFT, HOP_SIZE, FINE_WIN)
    log_blur = _log_mag(wav, BLUR_FFT, HOP_SIZE, BLUR_WIN)
...
def load_or_compute(cache_path: str, feat_fn, paths: list, desc: str):
    if os.path.exists(cache_path):
        ...
        print('WARNING:   Cache looks degenerate (std≈0) - recomputing: %s' % (cache_path,))
...
def _config_fingerprint_for(feature_name: str | None):
    ...
    cfg = dict(**base, fine_fft=FINE_FFT, fine_win=FINE_WIN, blur_fft=BLUR_FFT, blur_win=BLUR_WIN)
...
def main():
    ...
    print('=' * 70)
    print('SPECTRAL DETAIL — SEPARABILITY EVALUATION')
    print('run_id: %s' % (run_id,))
    print('=' * 70)

    print('Fine   STFT : FFT=%d, win=%d, hop=%d -> %d bins' % (FINE_FFT, FINE_WIN, HOP_SIZE, FINE_BINS))
    print('Blurry STFT : FFT=%d, win=%d, hop=%d -> %d bins (upsampled to %d)' % (BLUR_FFT, BLUR_WIN, HOP_SIZE, BLUR_BINS, FINE_BINS))
    ...
    features = [("SpectralDetail", extract_spectral_detail)]
    ...
    for feat_name, feat_fn in features:
        ...
        remake_train = bool(REMAKE_CACHE_SPECTRAL_TRAIN)
        remake_eval  = bool(REMAKE_CACHE_SPECTRAL_EVAL)
